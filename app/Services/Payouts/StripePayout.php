...
class StripePayout
{
    ...
    public function getAvailableFunds()
    {
        ...
        return array('amount' => $response->available[0]->amount, 'currency' => $response->available[0]->currency);
    }

    protected function getVerificationUpdateData($request)
    {
        $user = $this->getCurrentUser();
        if (!$user) {
            $user = $request->user;
    ...
    protected function updateStripeAccount($stripe_id, $verification)
    {
        try {
            $recipient = \Stripe\Account::update($stripe_id, $verification);
            return array(
                'status' => true,
                'recipient' => $recipient,
            );
    ...
    public function updatePayoutPreference($stripe_id, $request)
    {
        $verification = $this->getVerificationUpdateData($request);
        $recipient_data = $this->updateStripeAccount($stripe_id, $verification);
        if (!$recipient_data['status']) {
            return array(
                'status' => false,
                'status_message' => $recipient_data['status_message'],
        ...
        try {
            $recipient->external_accounts->create(
                array("external_account" => $request->stripe_token, "default_for_currency" => true)
            );
        } catch (\Exception $e) {
            return array(
                'status' => false,
