...
class AppServiceProvider extends ServiceProvider
{
	...
	public function boot() {
		if(!defined('EMAIL_LOGO_URL')) define('EMAIL_LOGO_URL', 'images/logo.png');
		if(!defined('LOGIN_USER_TYPE')) define('LOGIN_USER_TYPE', request()->segment(1));
		...
		if (env('DB_DATABASE') != '') {
			...
			if (Schema::hasTable('site_settings')) {
				...
				if(!defined('LOGO_URL')) define('LOGO_URL', 'images/logos/' . $site_settings[3]->value);
				if(!defined('PAGE_LOGO_URL')) define('PAGE_LOGO_URL', 'images/logos/' . $site_settings[4]->value);
				View::share('favicon', url('images/logos/' . $site_settings[5]->value).'?v='.str_random(5));
				View::share('site_name', $site_settings[0]->value);
				if(!defined('SITE_NAME')) define('SITE_NAME', $site_settings[0]->value);

				if(!defined('MANUAL_BOOK_CONTACT')) define('MANUAL_BOOK_CONTACT', '+'.$site_settings[9]->value.' '.$site_settings[8]->value);

				if(!defined('PAYPAL_CURRENCY_CODE')) define('PAYPAL_CURRENCY_CODE', $site_settings[1]->value);
				if(!defined('PHP_DATE_FORMAT')) define('PHP_DATE_FORMAT','Y-m-d');
				if(!defined('SITE_URL')) define('SITE_URL',$site_settings[10]->value);
				if(!defined('Driver_Km')) define('Driver_Km', $site_settings[6]->value);
			}
			...
			if (Schema::hasTable('api_credentials')) {
				...
				if(!defined('MAP_KEY')) define('MAP_KEY', $google_map_result->where('name','key')->first()->value);
				if(!defined('MAP_SERVER_KEY')) define('MAP_SERVER_KEY', $google_map_result->where('name','server_key')->first()->value);
				View::share('map_key', $google_map_result->where('name','key')->first()->value);

                // For HERE Key
				$here_result = $api_credentials->where('site', 'Here');
				if(!defined('HERE_REST_KEY')) define('HERE_REST_KEY', $here_result->where('name','rest_api_key')->first()->value);
				if(!defined('HERE_JS_KEY')) define('HERE_JS_KEY', $here_result->where('name','javascript_api_key')->first()->value);
                View::share('here_js_key', $here_result->where('name','javascript_api_key')->first()->value);

				//For facebook
				$facebook_result = $api_credentials->where('site', 'Facebook');
				if(!defined('FB_CLIENT_ID')) define('FB_CLIENT_ID', $facebook_result->where('name','client_id')->first()->value);

	        	// Share Google Credentials
	        	$google_result =  $api_credentials->where('site','Google');
	        	if(!defined('GOOGLE_CLIENT_ID')) define('GOOGLE_CLIENT_ID', $google_result->where('name','client_id')->first()->value);

				// For Twillo Key
				$twillo_result = $api_credentials->where('site', 'Twillo');

				if(!defined('TWILLO_SID')) define('TWILLO_SID', $twillo_result->where('name','sid')->first()->value);
				if(!defined('TWILLO_TOKEN')) define('TWILLO_TOKEN', $twillo_result->where('name','token')->first()->value);
				if(!defined('TWILLO_FROM')) define('TWILLO_FROM', $twillo_result->where('name','from')->first()->value);
				...
				if(!defined('FB_URL')) define('FB_URL', $fb->getUrlLogin());
			}
	...
	protected function setPaymentConfig()
	{
		...
		if(!defined('PAYPAL_ID')) define('PAYPAL_ID', payment_gateway('mode','paypal_id'));
		if(!defined('PAYPAL_MODE')) define('PAYPAL_MODE', ($paypal_mode == 'sandbox') ? 0 : 1);
		if(!defined('PAYPAL_CLIENT_ID')) define('PAYPAL_CLIENT_ID', payment_gateway('client','Paypal'));

		if(!defined('STRIPE_KEY')) define('STRIPE_KEY', payment_gateway('publish','Stripe'));
		if(!defined('STRIPE_SECRET')) define('STRIPE_SECRET', payment_gateway('secret','Stripe'));
