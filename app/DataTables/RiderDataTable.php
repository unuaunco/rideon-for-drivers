...
class RiderDataTable extends DataTable
{
    ...
    {
        $users = DB::Table('users')->select('users.id as id', 'users.first_name', 'users.last_name','users.email','users.country_code',DB::raw('CONCAT("+", users.country_code, " ", users.mobile_number) AS full_mobile_number'), 'users.status','users.created_at',DB::raw('CONCAT("XXXXXX",Right(users.mobile_number,4)) AS hidden_mobile'))->where('user_type','Rider')->groupBy('id');
        return $users;
    }
