...
class TripsDataTable extends DataTable
{
    ...
    public function query(Trips $model)
    {
        $trips =  $result = $model->
                        ...
                        ->leftJoin('delivery_orders', function($join) {
                                $join->on('trips.request_id', '=', 'delivery_orders.ride_request');
                        })
                        ->leftJoin('merchants', function($join) {
                                $join->on('delivery_orders.merchant_id', '=', 'merchants.id');
                        })
                        ->select([
                            'trips.id as id',
                            'trips.trip_type as trip_type',
                            'trips.begin_trip as begin_trip',
                            'trips.pickup_location as pickup_location',
                            'trips.drop_location as drop_location',
                            'driver.first_name as driver_name',
                            'rider.first_name as rider_name', 
                            DB::raw('CONCAT(currency.symbol, trips.total_fare) AS total_amount'),
                            'trips.total_fare as total',
                            'trips.status as status',
                            'car_type.car_name as car_name',
    ...
    protected function getColumns()
    {
        ...
        $col_list_1 = [
            ...
            ['data' => 'trip_type', 'name' => 'trips.trip_type', 'title' => 'Trip Type'],
            ['data' => 'merchant_name', 'name' => 'merchants.name', 'title' => 'Merchant'],
