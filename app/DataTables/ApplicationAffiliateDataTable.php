...
class ApplicationAffiliateDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($application) {
                $download = (LOGIN_USER_TYPE=='company' || auth('admin')->user()->can('update_affiliate')) ? '<a href="'.$application->pdf.'" class="btn btn-xs btn-primary" target="_blank"><i class="glyphicon glyphicon-download-alt"></i></a>&nbsp;' : '';
                if ($application->status == 'Active') 
                    $check = '';
                else
                    $check = (auth()->guard('company')->user()!=null || auth('admin')->user()->can('delete_affiliate')) ? '<a data-href="'.url(LOGIN_USER_TYPE.'/active_affiliate_application/'.$application->id).'" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#confirm-active"><i class="glyphicon glyphicon-check"></i></a>&nbsp;':'';
    ...
    public function query(User $model)
    {
        ...
        $users = DB::Table('applications')->select('applications.id as id', 'applications.pdf', 'users.first_name', 'users.last_name','users.email','users.country_code','users.mobile_number', 'users.status', 'users.created_at',DB::raw('CONCAT("XXXXXX",Right(users.mobile_number,4)) AS hidden_mobile'))
            ...
            })->where('applications.type','Affiliate')
            ->groupBy('id');
