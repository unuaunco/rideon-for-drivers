    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($plans) {
                $edit = (LOGIN_USER_TYPE=='company' || auth('admin')->user()->can('update_subscriptions')) ? '<a href="'.url('admin/subscriptions/edit_plan/'.$plans->id).'" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
                $delete = (LOGIN_USER_TYPE=='company' || auth('admin')->user()->can('delete_subscriptions')) ? '<a data-href="'.url('admin/subscriptions/delete_plan/'.$plans->id).'" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#confirm-delete"><i class="glyphicon glyphicon-trash"></i></a>&nbsp;' : '';
                return $edit.$delete;
            });
    }
