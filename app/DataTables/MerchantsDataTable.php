...
class MerchantsDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        ->addColumn('action', function ($merchant) {
            ...
            $edit = (LOGIN_USER_TYPE=='company' || auth('admin')->user()->can('update_merchant')) ? '<a href="'.url(LOGIN_USER_TYPE.'/edit_merchant/'.$merchant->id).'" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
            $delete = (auth()->guard('company')->user()!=null || auth('admin')->user()->can('delete_merchant')) ? '<a data-href="'.url(LOGIN_USER_TYPE.'/delete_merchant/'.$merchant->id).'" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#confirm-delete"><i class="glyphicon glyphicon-trash"></i></a>&nbsp;':'';
    ...
    public function query(Merchant $model)
    {
        return $model->join('merchants_integration_types as integration', function($join) {
            ...
            $join->on('users.id', '=', 'merchants.user_id');
        })
        ->select([
            'merchants.id as id',
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('integration')->name('integration.name')->searchable(false),
            Column::make('owe_fees')->title('Owe fees')->searchable(false),
