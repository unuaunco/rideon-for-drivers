...
class UserController extends Controller
{
	...
	public function login_affiliate_driver(Request $request)
	{
		$rules = array(
			'email' => 'required|email',
			'password' => 'required|min:6',
		...
		$user = User::where("email", $request->email)->where('user_type','Affiliate')->first();
		...
		if($user) {
			...
			// 	if (in_array($plan->plan_name, array('Regular','Founder','Executive')))
			// 	{
						if (Auth::guard('web')->attempt(['email' => $request->email, 'password' =>$request->password, 'user_type' => 'Affiliate'])) {
    ...
    public function login_merchant(Request $request)
	{
		...
		$user = User::where("email", $request->email)->where('user_type','Merchant')->first();


		if($user) {
            if (Auth::guard('web')->attempt(['email' => $request->email, 'password' =>$request->password, 'user_type' => 'Merchant'])) {
                return redirect()->intended('merchants/home');
            }
            return back()->withErrors(['password' => 'Invalid credentials'])->withInput();
		}
		return back()->withErrors(['password' => 'Invalid credentials'])->withInput();
