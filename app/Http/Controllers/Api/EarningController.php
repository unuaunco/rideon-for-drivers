...
class EarningController extends Controller
{
	...
	public function earning_chart(Request $request)
	{
		...
		$last_trip = Trips::where('driver_id', $driver_details->id)->where('status', 'Completed')->whereIn('request_id', function ($query) use ($driver_details) { $query->select('id')->from('request')->where('deleted_at', null)->where(
'driver_id', $driver_details->id);})->orderBy('id', 'DESC')->first();
	...
	public function afterPayment(Request $request)
	{
		...
		if($trip->status != 'Payment' && $trip->status != 'Rating') {
			return response()->json([
				'status_code' => '0',
