        ...
        try {
            ...
            if ($request->hasFile('document')) {
                ...
                if (App::environment(['production'])) {
                    $target_dir = 'images/users/' . $user_id;
                } else {
                    $target_dir = '/images/users/' . $user_id . '/';
                }
                ...
                if (App::environment(['production'])) {
                    $file_name = "payout_document_" . $user_id . time() . "." . $extension;
                    $upload_result = Storage::disk('do_spaces')->putFileAs($target_dir, $image, $file_name, 'public');

                    if (!$upload_result) {
                        return response()->json([
                            'status_code' => '0',
                            'status_message' => 'Can\'t upload document',
                        ]);
                    }

                    $document_path = Storage::disk('do_spaces')->url($target_dir . '/' . $file_name);

                    $docfile = Storage::disk('local')->putFileAs('doc', $image, $file_name);
                    $docfile = Storage::disk('local')->getAdapter()->applyPathPrefix($docfile);
                } else {
                    $file_name = "payout_document_" . $user_id . time() . "." . $extension;
            ...
            if ($payout_method == 'Stripe') {
                ...
                $payout_preference = PayoutPreference::where('user_id', $user_id)->count();
            ...
            return response()->json([
                'status_code' => '1',
                'status_message' => 'Payout details cannot be added right now. Please, try later or get contact support',
            ]);
        }
    }
