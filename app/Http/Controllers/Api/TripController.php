...
class TripController extends Controller
{
	...
	protected function getTripDetails($trip_id,$user)
	{
        ...
        $ride = RideRequest::where('id', $trip->request_id)->first();
		$driver = $trip->driver;
	...
	public function get_pending_trips(Request $request)
	{
        ...
        // $trips = Trips::with('car_type')
        //     ->where('trips.driver_id', $user_details->id)
        //     ->orderBy('trips.id','DESC')
        //     ->whereNotIn('status',['Completed','Cancelled'])
        //     ->join('request', 'request.id', '=', 'trips.request_id')
        //     ->where('request.deleted_at', null)
        //     ->get();
        $trips = Trips::with('car_type')->where('trips.driver_id', $user_details->id)->orderBy('trips.id','DESC')->whereNotIn('status',['Completed','Cancelled'])->whereIn('request_id', function ($query) use ($user_details) { $query->select('id')->from('request')->where('deleted_at', null)->where(
'driver_id', $user_details->id);})->get();
	...
	public function get_completed_trips(Request $request)
	{
        ...
        $trips = Trips::with('car_type')->where('trips.driver_id', $user_details->id)->orderBy('trips.id','DESC')->whereIn('trips.status',['Completed'])->whereIn('request_id', function ($query) use ($user_details) { $query->select('id')->from('request')->where('deleted_at', null)->where(
'driver_id', $user_details->id);})->paginate($this->paginate_limit)->toJson();
