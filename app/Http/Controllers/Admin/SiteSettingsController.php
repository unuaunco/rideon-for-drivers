...
class SiteSettingsController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		$rules = array(
			...
			'max_self_assign_orders' => 'required',
			'growsurf_campaign_id' => 'required',
			'regular_driver_booking_fee' => 'required',
			'tax_gst' => 'required',
		);
		...
		$attributes = array(
			...
			'max_self_assign_orders' => 'Max self Assign Orders',
			'growsurf_campaign_id' => 'GrowSurf Campaign ID',
			'regular_driver_booking_fee' => 'Booking Fee for Regular Driver',
			'tax_gst' => 'Tax of GST volume',
		);
		...
		SiteSettings::where(['name' => 'max_self_assign_orders'])->update(['value' => $request->max_self_assign_orders]);
		SiteSettings::where(['name' => 'growsurf_campaign_id'])->update(['value' => $request->growsurf_campaign_id]);
		SiteSettings::where(['name' => 'regular_driver_booking_fee'])->update(['value' => $request->regular_driver_booking_fee]);
		SiteSettings::where(['name' => 'tax_gst'])->update(['value' => $request->tax_gst]);
        SiteSettings::where(['name' => 'driver_submission_url'])->update(['value' => $request->driver_submission_url]);
        SiteSettings::where(['name' => 'manual_surchange'])->update(['value' => $request->manual_surchange]);
