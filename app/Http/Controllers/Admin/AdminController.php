...
class AdminController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['total_driver'] = DB::table('users')->where('user_type','Driver')
                                ...
                                ->count();
        $data['total_rider'] = DB::table('users')->where('user_type','Rider')->count();
        $data['today_driver_count'] = DB::table('users')->whereDate('created_at', '=', date('Y-m-d'))
                                        ->where(function($query)  {
                                        ...
                                        ->count();
        $data['today_rider_count'] = DB::table('users')->whereDate('created_at', '=', date('Y-m-d'))->where('user_type','Rider')->count();
        ...
        if(LOGIN_USER_TYPE=='company') {  //if login user is company then revenue calculated from company trips
            ...
            $data['today_revenue'] = DB::table('trips')->whereDate('created_at', '=', date('Y-m-d'))
                ->where('status','Completed')
                ->whereHas('driver',function($q1){
                    $q1->where('company_id',Auth::guard('company')->user()->id);
                })
        ...
        $data['today_trips'] = DB::table('trips')->whereDate('created_at', '=', date('Y-m-d'))
                ->where(function($query)  {
                ...
                ->count();
        $data['total_trips'] = DB::table('trips')->
                                where(function($query)  {
                                    if(LOGIN_USER_TYPE=='company') {  //if login user is company then get only company driver's trip
                                        $query->whereHas('driver',function($q1){
                                            $q1->where('company_id',Auth::guard('company')->user()->id);
                                ...
                                ->count();
        $data['total_success_trips'] = DB::table('trips')->where('status','Completed')
        ->where(function($query)  {
            if(LOGIN_USER_TYPE=='company') {  //if login user is company then get only company driver's trip
                $query->whereHas('driver',function($q1){
                    $q1->where('company_id',Auth::guard('company')->user()->id);
        ...
        if(LOGIN_USER_TYPE=='company') {   //if login user is company then revenue is sum of trip amount
            ...
            $data['total_revenue'] = DB::table('trips')->where('status','Completed')
                ->whereHas('driver',function($q1){
                    $q1->where('company_id',Auth::guard('company')->user()->id);
                })->get();
            ...
            $data['total_revenue'] = DB::table('trips')->where('status','Completed')->get();
            $data['total_revenue'] = $data['total_revenue']->sum('commission');
        }

        if(LOGIN_USER_TYPE=='company') {
            $data['admin_paid_amount'] = DB::table('trips')->where('status','Completed')
            ...
            $data['admin_pending_amount'] = DB::table('trips')->where('status','Completed')
                ->where('driver_payout','>',0)
                ->where('payment_mode','<>','Cash')
                ->whereHas('driver',function($q){
                    $q->where('company_id',Auth::guard('company')->user()->id);
    ...
    public function sendBirdChat(){
        $admin_data = Auth::guard('admin')->user();

        return redirect()->away('https://chat.rideon.co/chat.html?userid='.$admin_data->mobile_number.'&nickname='.$admin_data->username);
    }
    ...
    public function update(Request $request)
    {
        if(!$_POST) {
            ...
            if(!Auth::guard('admin')->user()->can('manage_admin')) {
                ...
                return redirect('admin/home_delivery');
            }
        ...
        if ($request->user_type == 'Company') {
            ...
            if(isset($admin) && $admin->status != 'Inactive') {
                if(Auth::guard('admin')->attempt(['username' => $request->username, 'password' => $request->password])) {
                    if(auth('admin')->user()->can('manage_application') && !auth('admin')->user()->can('view_delivery')){
                        return redirect()->intended('admin/application_driver');
                    }
                    //return redirect()->intended('admin/dashboard'); // Redirect to dashboard page
                    return redirect()->intended('admin/home_delivery');
                }
