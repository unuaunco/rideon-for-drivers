...
{    
    ...
    public function get_send_users(Request $request)
    {
        $type=$request->type;
        if($type == "Company") {
            $company_details=Company::select('id','name','country_code','mobile_number', DB::raw('"Company" as user_type'))->where('status','Active')->where('id','!=',1)->get();
            return $company_details->toJson();
        }

        $user_details=DB::table('users')->select('id','first_name','last_name','country_code','mobile_number','user_type','referral_code')->where('status','Active');
    ...
    {
        return DB::table('merchants')->select('id','name')->get()->toJson();
    }
     ...
     * @return users list
     */
    public function get_send_drivers()
    {
        return DB::table('users')->select('id','first_name','last_name','country_code','mobile_number')
            ->where('user_type','driver')->where('status','Active')->get()->toJson();
    }

    /**
     * Get merchants addresses
     *
     * @return users list
     */
    public function get_merchant_address(Request $request)
    {
        $merchant_id = $request->id;
        $merchant_details = Merchant::where('id',$merchant_id)->first();
        $address = DriverAddress::where('user_id', $merchant_details->user_id)->first();
        
        $full_address = $address->address_line1 . " " . $address->address_line2 . ", " . $address->city . " " . $address->state . " " . $address->postal_code;
