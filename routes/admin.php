...
Route::group(['prefix' => (LOGIN_USER_TYPE=='company')?'company':'admin', 'middleware' =>'admin_guest'], function () {
	...
	Route::redirect('/',LOGIN_USER_TYPE.'/home_delivery_dashboard');
	Route::get('dashboard', 'AdminController@index');
    Route::get('chat', 'AdminController@sendBirdChat');
	...
	Route::group(['middleware' => 'admin_can:manage_send_message'], function() {
        ...
        Route::get('get_send_drivers', 'SendmessageController@get_send_drivers');
        Route::get('get_merchant_address', 'SendmessageController@get_merchant_address');
	});
	...
	// Manage Community Leader
	Route::get('community_leader', 'CommunityLeaderController@index')->middleware('admin_can:view_driver');
	Route::get('community_leader/{id}', 'CommunityLeaderController@detail')->middleware('admin_can:view_driver');
	Route::match(array('GET', 'POST'), 'add_community_leader', 'CommunityLeaderController@add')->middleware('admin_can:create_driver');
    ...
    // Manage Community Leader
	Route::get('affiliate', 'AffiliateController@index')->middleware('admin_can:view_driver');
	Route::get('affiliate/{id}', 'AffiliateController@detail')->middleware('admin_can:view_driver');
	Route::match(array('GET', 'POST'), 'add_affiliate', 'AffiliateController@add')->middleware('admin_can:create_driver');
	Route::match(array('GET', 'POST'), 'edit_affiliate/{id}', 'AffiliateController@update')->middleware('admin_can:update_driver');
	Route::match(array('GET', 'POST'), 'delete_affiliate/{id}', 'AffiliateController@delete')->middleware('admin_can:delete_driver');

    //Manage Applications
    Route::get('application_driver', 'ApplicationController@driver');
    Route::get('application_merchant', 'ApplicationController@merchant');
    Route::get('application_affiliate', 'ApplicationController@affiliate');
    ...
    Route::get('home_delivery_dashboard','HomeDeliveryController@ordersDashboard');
	...
	//Manage Shifts
	Route::get('shifts', 'ShiftController@view');
	Route::match(array('GET', 'POST'),'add_shift', 'ShiftController@add');
	Route::match(array('GET', 'POST'),'edit_shift/{id}', 'ShiftController@update');
	Route::match(array('GET', 'POST'),'delete_shift/{id}', 'ShiftController@delete');
	Route::match(array('GET', 'POST'), 'import_shifts', 'ShiftController@importShifts');
    ...
    Route::get('invoice_transactions', 'TransactionsController@get_invoices');
    Route::get('payout_transactions', 'TransactionsController@get_payouts');
    Route::get('payout_statistics', 'TransactionsController@payoutStatistics');
    ...
    Route::post('import_affiliates', 'AffiliateController@import_affiliates');
	...
	Route::group(['middleware' =>  'admin_can:manage_driver_payments'], function() {
        ...
        Route::post('make_custom_payout', 'PayoutController@custom_payout_to_driver');
	});
