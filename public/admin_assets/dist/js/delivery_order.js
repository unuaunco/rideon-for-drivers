...
app.controller('delivery_order', ['$scope', '$http', '$compile', '$filter', function($scope, $http, $compile, $filter) {
    ...
    $(function() {
        ...
        if ($scope.date_time == '') {
            $scope.date_time = moment().tz('Australia/Melbourne').format('ha z');
        }
        var today = new Date().toLocaleString("en-US", { timeZone: "Australia/Melbourne" });
        //console.log(today);
        $('#input_date_time').datetimepicker({
            format: 'YYYY-MM-DD HH:mm',
            //minDate: new Date().toLocaleString("en-US", { timeZone: "Australia/Melbourne" }),
            ignoreReadonly: true,
            sideBySide: true,
    ...
    var mapIcons = {
        ...
        Black: APP_URL + '/images/car_black.png'
    }
    var googleMarker = [];
    var map;
