...
<div class="manual-booking content-wrapper" ng-controller='delivery_order'>
    ...
    {!! Form::open(['method'=>'POST','url' => LOGIN_USER_TYPE.'/edit_home_delivery/'.$result->id, 'class' =>
    'form-horizontal delivery_adding','id'=>'delivery_order','name'=>'deliveryAddForm']) !!}
    ...
    {!! Form::hidden('driver_id', $result->driver_id, ['id' => 'driver-id']) !!}

    @endif

    @if($result->created_at)

    {!! Form::hidden('created_at', $result->created_at, ['id' => 'created-time']) !!}

    @endif   

    <section class="content">
        <h4>Edit Delivery Order #{{$result->id}}</h4>
        <h5>Customer Data</h5>
        <div class="row">
            <div class="col-md-3" ng-init="country_code={{($country_code)}}">
                <select class='form-control selectpicker' data-live-search="true" id="input_country_code"
                    name='country_code' ng-model="country_code">
            ...
            <div class="col-md-3 number-field">
                ...
                <div class="col-md-7 col-lg-8">
                    {!! Form::hidden('customer_phone_number', @$result->customer_phone_number, ['id' =>
                    'customer_phone_number']) !!}
                    {!! Form::text('mobile_number', @$mobile_number, ['class' => 'form-control', 'id' =>
                    'input_mobile_number', 'placeholder' => 'Phone No', 'autocomplete' => 'off']) !!}
                    <span class="text-danger error_msg">{{ $errors->first('mobile_number') }}</span>
                </div>
            </div>
            <div class="col-md-2 form-group m-0">
                {!! Form::hidden('customer_name', @$result->customer_name, ['id' => 'customer_name']) !!}
                {!! Form::text('first_name', @$first_name, ['class' => 'form-control', 'id' => 'input_first_name',
                'placeholder' => 'First Name', 'autocomplete' => 'off']) !!}
                <span class="text-danger error_msg">{{ $errors->first('first_name') }}</span>
            </div>
            <div class="col-md-2 form-group m-0">
                {!! Form::text('last_name', @$last_name, ['class' => 'form-control', 'id' => 'input_last_name',
            ...
            <div class="col-md-4 location-form">
                ...
                <div class="row pick-location clearfix">
                    <div class="col-md-12" ng-init='pick_up_latitude = ""'>
                        ...
                        !!}
                        {!! Form::hidden('pick_up_longitude', @$result->pick_up_longitude, ['id' =>
                        'pick_up_longitude']) !!}
                        {!! Form::text('pick_up_location', @$result->pick_up_location, ['class' => 'form-control
                        change_field', 'id' => 'input_pick_up_location', 'placeholder' => 'Pick Up Location',
                        'autocomplete' => 'off']) !!}
                        <span
                            class="text-danger error_msg error_pick_up_location">{{ $errors->first('pick_up_location') }}</span>
                    </div>
                </div>
                <div class="row pick-location clearfix">
                    <div class="col-md-12">
                        {!! Form::hidden('drop_off_latitude', @$result->drop_off_latitude, ['id' =>
                        'drop_off_latitude']) !!}
                        {!! Form::hidden('drop_off_longitude', @$result->drop_off_longitude, ['id' =>
                        'drop_off_longitude']) !!}
                        {!! Form::text('drop_off_location', @$result->drop_off_location, ['class' => 'form-control
                        change_field', 'id' => 'input_drop_off_location', 'placeholder' => 'Drop Off Location',
                        'autocomplete' => 'off']) !!}
                        <span
                            class="text-danger error_msg error_drop_off_location">{{ $errors->first('drop_off_location') }}</span>
                    </div>
                </div>
                <div class="row clearfix">
                    <div class="col-md-12" ng-init='date_time = ""'>
                        {!! Form::text('estimate_time', @$estimate_time, ['class' => 'form-control change_field', 'id'
                        => 'input_date_time', 'placeholder' => 'Estimate time','ng-cloak']) !!}
                        <span class="text-danger error_msg"></span>
                    </div>
                </div>
                <div class="row clearfix">
                    <div class="col-md-12">
                        {!! Form::number('fee', @$result->fee, ['class' => 'form-control', 'id' => 'input_fee',
                        'placeholder' => '0.00', 'autocomplete' => 'off',"step" => "0.01"]) !!}
                        <span class="text-danger error_msg">{{ $errors->first('fee') }}</span>
                    </div>
                </div>
                <div class="row clearfix">
                    <div class="col-md-12">
                        {!! Form::textArea('order_description', @$result->order_description, ['class' => 'form-control
                        change_field', 'id' => 'input_order_description', 'placeholder' => 'Order description']) !!}
                        <span class="text-danger error_msg"></span>
                    </div>
                </div>
            </div>
            <div class="col-md-8 map-wrap">
                <div class="map-route-option">
                    <div>
                        <div class="clearfix map_zoom_level">
                            ...
                            <div class="col-md-6" ng-init="map_radius=0">
                                <select class='form-control' id='input_map_zoom' name='map_zoom' ng-model="map_radius"
                                    ng-change="map_zoom(map_radius)">
                    ...
                    <div class="fare-btn clearfix">
                        <button type="button" class="btn btn-primary submit_button change_field" disabled
                            ng-disabled="deliveryAddForm.$invalid && page_loading==0" ng-click="submitForm($event);">
                            Submit
                        </button>
                    </div>
                </div>
